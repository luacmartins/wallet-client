const Input = ({
  ...
  return (
    <>
      <div
        className={`flex flex-col relative border rounded bg-white px-3 w-full font-semibold py-2
      ...
      >
        <label
          htmlFor={name}
          className={`${variant === 'inside' ? 'text-sm px-1 ' : ''}
                  ${
                    variant === 'floating'
                      ? 'absolute text-sm transform -translate-y-5 bg-white px-1'
                      : ''
                  }
      ...
      {error[name] && (
        <span className='mb-1 -mt-1 text-sm font-normal text-red-600 rounded'>
          {error[name].message}
        </span>
      )}
    </>
